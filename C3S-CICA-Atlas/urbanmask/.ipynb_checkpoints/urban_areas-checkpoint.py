...
Altitude difference (m) respects the maximum and minimum elevation of the urban cells.
    ...
    def select_urban_vicinity(
        ...
        non_urban_cells = 0
        counter = 0
        while non_urban_cells <= urban_cells * scale and counter<=10:
        ...
        if lon2d.ndim == 1:
            ...
            for lon in range(len(lon2d)):
                for lat in range(len(lat2d)):
                    square = Polygon([
                        (lon2d[lon] - dist_lon, lat2d[lat] - dist_lat),  # bottom-left corner
                        (lon2d[lon] + dist_lon, lat2d[lat] - dist_lat),  # bottom-right corner
                        (lon2d[lon] + dist_lon, lat2d[lat] + dist_lat),  # top-right corner
                        (lon2d[lon] - dist_lon, lat2d[lat] + dist_lat),  # top-left corner
                    ])
    ...
    def plot_static_variables(self, ds_sftuf, ds_orog, ds_sftlf,
        ...
        if urban_areas:
            for k in range(3):
                ...
                gdf_urban, gdf_non_urban=Urban_vicinity.plot_urban_polygon(self, urban_areas, axes[1, k])
    ...
    def plot_static_variables_with_vegetation(self, ds_sftuf, ds_cvl , ds_cvh,
                             sftuf_mask, urban_areas = None,
                            ):
        """
        Plot fix variables including urban fraction, orography, and land-sea mask.
        ...
        im2 = axes[0, 1].pcolormesh(ds_cvl.lon, ds_cvl.lat,
                                    ds_cvl['cvl'], 
                                    cmap='YlGn',
                                    vmin = np.nanmin(ds_cvl['cvl']), 
                                    vmax = np.nanmax(ds_cvl['cvl'])
        )
        fig.colorbar(im2, ax=axes[0, 1], orientation='vertical')
        axes[0, 1].set_title('Low vegetation cover')
        axes[0, 1].coastlines()
        
        im3 = axes[0, 2].pcolormesh(ds_cvh.lon, ds_cvh.lat,
                                    ds_cvh["cvh"],
                                    cmap='YlGn',
                                    vmin = np.nanmin(ds_cvh['cvh']), 
                                    vmax = np.nanmax(ds_cvh['cvh']))
        fig.colorbar(im3, ax=axes[0, 2], orientation='vertical')
        axes[0, 2].set_title('High vegetation cover')
        axes[0, 2].coastlines()
        ...
        im2 = axes[1, 1].pcolormesh(ds_cvl.lon, ds_cvl.lat,
                                    ds_cvl['cvl'], 
                                    cmap='YlGn',
                                    vmin = np.nanmin(ds_cvl['cvl']), 
                                    vmax = np.nanmax(ds_cvl['cvl'])
        ...
        fig.colorbar(im2, ax=axes[1, 1], orientation='vertical')
        axes[1, 1].set_title('Low vegetation cover')
        axes[1, 1].coastlines()
        
        im3 = axes[1, 2].pcolormesh(ds_cvh.lon, ds_cvh.lat,
                                    ds_cvh["cvh"],
                                    cmap='YlGn',
                                    vmin = np.nanmin(ds_cvh['cvh']), 
                                    vmax = np.nanmax(ds_cvh['cvh']))
        fig.colorbar(im3, ax=axes[1, 2], orientation='vertical')
        axes[1, 2].set_title('High vegetation cover')
        axes[1, 2].coastlines()
